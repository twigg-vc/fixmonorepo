...
The canonical home of **`b.txt`**.

`b.txt` used to be one line in a directory listing, sharing a repository
...
This repository contains `b.txt`. It does not contain anything else.
...
```sh
git clone <this repo>
cp b.txt $WHEREVER_IT_USED_TO_BE
```

Its original location was `subfolder/b.txt`, though that is an implementation detail
of the consumer and this repository makes no guarantees about it.

## Dependencies

See `atom.json`. If `b.txt` references code that lives in a sibling
repository, coordinate the change across both repositories, and their
CHANGELOGs, and their release tags, and their reviewers.

## Contributing

1. Open an issue describing the change to `b.txt`.
...
6. Update the CHANGELOG here to say `- bump b.txt`.
